...
# from compressai.models.v_mamba.vim.rope import *
# from rope import *
import random
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, padding=0,
                 flatten=True):
        super().__init__()
...
N=128
vm = VisionMamba(
    patch_size=3, stride=1, embed_dim=N // 2, padding=1, depth=6, rms_norm=True,
    residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=False, if_rope=False,
    if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_devide_out=True,
    num_classes=0, channels=N // 2).cuda()

print(vm)
x_input = torch.randn([16, 64, 32, 32]).cuda()
out = vm(x_input)
print(out.shape)